...
func (bmhnode *BMHNode) CreateISO() error {
	...
	err = bmhnode.CreateFileFromString(bmhnode.NetworkConfig, iso_custom_scripts_path, "netplan")
	if err != nil {
	...
	if err != nil {
		errMessage = "Error:Failed to create Netplan file"
		logger.Log.Error(errMessage, zap.Error(err))
		return fmt.Errorf(errMessage+"%+v", err)
	}

	//Cloud Init
	err = bmhnode.CreateFileFromString(bmhnode.CloudInit, iso_custom_scripts_path, "cloudinit")
	if err != nil {
		errMessage = "Error: Failed to create CloudInit file"
		logger.Log.Error(errMessage, zap.Error(err))
		return fmt.Errorf(errMessage+"%+v", err)
	}
